...
import os
import json
from trajectory_tool.monte_carlo_analysis.obj_def import Trajectory, aleInterpol
from trajectory_tool.ale_solar_system.time_utils import datetime_to_jd, jd_to_datetime


def plot_porkchop(X, Y, Z, title):
    x_grid, y_grid = np.meshgrid(np.linspace(min(X), max(X), 100), np.arange(min(Y), max(Y), 100))
    ...
    cs = plt.contourf(np.vectorize(jd_to_datetime)(x_grid), y_grid / 365.0, z_grid, levels)
...
if __name__ == "__main__":
    ...
    plot_porkchop(launch_dates, tof, delta_v, folder)
