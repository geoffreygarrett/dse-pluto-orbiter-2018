...
def angle_between(body1, body2, epoch, dt=datetime.timedelta(days=0), mind_direction=True):
...
def pseudo_distance(body1, body2, initial_epoch, dt=datetime.timedelta(days=0)):
    ...
    circ_dist = angle_between(body1, body2, initial_epoch, dt) * (r1_norm + r2_norm) * 0.5
...
for launch_date in launch_dates:
    if launch_date.month == 1 and launch_date.day == 1:
        save_cases(launch_date.year, cases)
        cases = ""
        ...
        for _ in range(8):  # In case pseudo_distance() depends on planets positions, iterate a bit for some convergence
            ...
            for i, (first, second) in enumerate(zip(itinerary, itinerary[1:])):
                pseudo_distances[i] = pseudo_distance(first, second,
                                                      launch_date + datetime.timedelta(days=365*sum(pseudo_times[:i])),
